...
RUN adduser -u 5000 -h /home/worker -D worker && \
    apk add --no-cache --update git yamllint curl docker openssh openssh-keygen && \
    apk add --no-cache --virtual .build-deps gcc musl-dev libffi-dev openssl-dev && \
    /usr/local/bin/python3 -m pip install --upgrade pip && \
    /usr/local/bin/python3 -m pip install jmespath netaddr passlib requests pywinrm bcrypt && \
